...
class NotionOAuthService {
    ...
    private let redirectScheme = "council"
    // Notion requires HTTPS redirect URIs. The Cloud Function bounces to council:// scheme.
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            let session = ASWebAuthenticationSession(
                ...
                if let errorParam = components.queryItems?.first(where: { $0.name == "error" })?.value {
                    continuation.resume(throwing: NotionOAuthError.denied(errorParam))
                    return
                }
...
enum NotionOAuthError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .denied(let reason): return "Notion access was denied: \(reason)"
