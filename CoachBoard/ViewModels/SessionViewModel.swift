...
class SessionViewModel: ObservableObject {
    ...
    @Published private(set) var selectedVoiceModel: TTSVoiceModel
    ...
    /// The agent ID to use for ElevenLabs API calls — user's clone or fallback to shared
    private var effectiveAgentId: String {
        resolvedAgentId ?? coach.elevenlabsAgentId
    }

    init(coach: Coach) {
        self.coach = coach
        self.activeCoaches = [coach]
        self.selectedVoiceModel = VoiceModelPreferences.shared.selectedModel
    ...
    func applySettings(model: LLMModel, voiceModel: TTSVoiceModel) async {
        let isModelChanged = model != coach.llmModel
        let isVoiceModelChanged = voiceModel != selectedVoiceModel
        guard isModelChanged || isVoiceModelChanged else { return }
        ...
        await prepareForConversationTransition(statusLabel: "Reconnecting")

        do {
            try await ElevenLabsAPIService.shared.updateAgent(
                agentId: effectiveAgentId,
                llmModel: isModelChanged ? model.rawValue : nil,
                ttsModelId: isVoiceModelChanged ? voiceModel.rawValue : nil
            )
            if isModelChanged {
                coach.llmModel = model
            }
            if isVoiceModelChanged {
                selectedVoiceModel = voiceModel
                VoiceModelPreferences.shared.setSelectedModel(voiceModel)
            }
            ...
            if activeCoaches.count > 1 {
                ...
                let prompt = buildSingleCoachPromptWithTranscript(transcript: transcriptSnapshot)
    ...
    private func buildMultiCoachPrompt(coaches: [Coach], transcript: [TranscriptMessage]) -> String {
        ...
        - The primary coach (\(coaches[0].name)) speaks with UNTAGGED text (no XML tags needed).
        """
        ...
        if !transcript.isEmpty {
            sections.append(transcriptSection(transcript: transcript, coaches: coaches,
                                              suffix: "Continue the conversation naturally. A new coach (\(coaches.last!.name)) has just joined — they may briefly acknowledge joining, then continue coaching."))
        }
    ...
    private func transcriptSection(transcript: [TranscriptMessage], coaches: [Coach], suffix: String) -> String {
        ...
        for msg in recentTranscript {
            let speaker = msg.role == .user ? "User" : (msg.coachName ?? coaches[0].name)
            lines.append("\(speaker): \(msg.content)")
        }
    ...
    private func parseMultiVoiceResponse(_ rawText: String) -> [VoiceSegment] {
        let primaryCoach = activeCoaches[0]
    ...
    private func beginBackgroundTransitionTask() {
        ...
        backgroundTaskId = UIApplication.shared.beginBackgroundTask(withName: "CoachBoardSession") { [weak self] in
            Task { @MainActor in
